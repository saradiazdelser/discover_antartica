...
ap.add_argument("-q", "--query", 
				nargs='+', # Must have at least 1 argument
				action="append",
				required=True,
...
for query in query_directory:
	...
	#try:
		dom.its_a_wonderful_database(
			...
			fasta_file=projectname+'_blast.fasta',
			text_file=projectname+'_domains.fasta',
			DOC=args.documentation,
			prosite_dat_file='discover_antartica/data/prosite.dat',
			prosite_doc_file='discover_antartica/data/prosite.doc'
			)
	#except:
